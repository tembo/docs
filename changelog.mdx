...
<Update label="2025-11-03" tags={["MCP", "Sentry", "Sandbox", "Documentation"]}>

## Model Context Protocol (MCP) Support

Tembo now supports [Model Context Protocol (MCP)](/features/mcp-servers), enabling agents to connect to external tools, APIs, and data sources through a standardized interface.
...
When you connect integrations, additional MCP servers become automatically available:

- **GitHub MCP** - Repository management, issues, and pull request operations
- **Linear MCP** - Issue tracking and project management
- **Sentry MCP** - Direct access to error data, stack traces, and issue status

### Custom MCP Configuration

You can now add custom MCP servers in your organization settings, supporting three server types:
...
The [Sentry integration](/integrations/sentry) now includes MCP server capabilities, allowing agents to:
- Query error data programmatically
- Retrieve detailed stack traces during debugging
- Check issue status and verify fixes
...
The [sandbox environment](/features/sandbox-environment) now supports the Nix ecosystem, enabling:
...
- Comprehensive workflows and automations documentation
- Updated agent references across all documentation
- Enhanced integration guides with detailed setup instructions
- Improved GitHub integration documentation structure
