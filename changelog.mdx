...
<Update
	label="2026-01-23"
	tags={["Sandbox Events", "Sandbox Logs", "Atlassian", "Jira", "MCP", "Automations", "Linear", "UI", "Audit Log", "Pull Requests"]}>

## New Sandbox events tab

You can now open a dedicated **Sandbox events** tab on every task to see what’s
happening inside the sandbox as it runs. Events stream live and are also loaded
from completed runs so you can review them later.

## Refreshed sandbox log viewer

Sandbox events are now easier to scan:
