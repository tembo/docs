...
- Tembo now works with [OpenCode](https://opencode.ai)! Only supports Anthropic models as of now with all opencode supported providers coming soon. Codex is also now supported in the new API but not form UI yet. You can view the supported agents here: [Agents](/features/agents).
- Support for the new Claude 4.5 Sonnet model

## Design

All new beautifully redesigned dashboard along with new settings pages

## Bug fixes and improvements

- Feedback loop on Github is now much faster
- Tagging is now required when leaving comments. This handles the scenario where you dont want tembo to respond to a comment meant for somone on your team or even a different bot user.
