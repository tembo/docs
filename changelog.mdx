...
New agent logs have been shipped for all agents including Claude Code, Codex, and OpenCode. Works with all models.

![Agent Logs Redesign](/images/agent-logs-redesign.png)

## Redesigned agent logs

Agent logs got a full visual refresh with clearer event grouping, richer tool input rendering
(diffs, commands, file writes), and easier-to-scan status/error states.

## Codex CLI support

Tembo now supports the Codex CLI as a first-class agent so you can run tasks directly with
Codex from the CLI.

## GPT-5.2 Codex model support

Added `gpt-5.2-codex` as an available model option for Codex-powered workflows.

## Reasoning level controls

You can now choose a reasoning level per task. The UI exposes supported levels, and the
agent key format `agent:model:reasoningLevel` works across app, API, and CLI.

![Tembo MAX](/images/tembo-max.png)

## Tembo MAX

One subscription all the coding agents. Introducing [Tembo MAX](https://tembo.io/max).
