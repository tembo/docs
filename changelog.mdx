...
## Start automations from blank

You can now create a new automation from a blank canvas, not just from a
template. That makes it faster to build custom workflows when you already know
...
## New Jira trigger: Issue Status Changed

Jira automations can now trigger when an issue changes status. You can use this
to kick off follow-up work automatically when issues move into states like **In
...
Supermemory is now available as a suggested MCP server, so you can connect
long-term memory and retrieval context to your agents in just a few clicks.
