...
## New model: Kimi K2.5 (Moonshot)

Kimi K2.5 is now available as a model option in Tembo.
Select it in **Settings → Agents** when choosing a model for your agent:
https://app.tembo.io/settings/agents

## Faster live task updates

Task pages now poll more aggressively while a task is queued or in progress, so
status changes (and new logs) show up faster without needing a manual refresh.
...
Learn more: [Feedback Loop](https://docs.tembo.io/features/feedback-loop)
