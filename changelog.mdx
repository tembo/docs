...
<Update label="2026-01-20" tags={["OSS Program", "Billing", "Agent Logs", "Sandbox", "UI"]}>

## OSS Program Support

Organizations participating in the OSS Program now have dedicated support with single repository enforcement and automatic billing bypass. This makes it easier for open source projects to use Tembo without worrying about credit management.

## Prorated Plan Upgrade Credits

When upgrading your plan, you now receive prorated credits for the remaining time on your current billing cycle. This ensures fair billing when moving to a higher tier plan.

## Enhanced Agent Logs

Agent logs now display a "thinking" indicator when AI models are reasoning through complex tasks, giving you better visibility into what's happening behind the scenes.

## File Icons in Diffs

File changes in agent logs now display language-specific icons, making it easier to identify file types at a glance when reviewing code modifications.

## Java Toolchain in Sandbox

The sandbox environment now includes Java development tools: JDK 21, Gradle, and Maven. Java projects can now be built and tested directly in Tembo's isolated environments.

## Improved Search

The @mention autocomplete search has been enhanced with better fuzzy matching, making it faster to find and reference files and integrations in your tasks.

## Bug Fixes

- Fixed duplicate subscription issues during Pro plan upgrades
- Improved GPT Codex credit tracking accuracy
- Enhanced Atlassian board detection with JQL queries
