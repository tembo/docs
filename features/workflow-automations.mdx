...
Tembo workflows can be triggered from provider webhooks. Use a consistent `triggerName` (we recommend `provider.event` style, e.g., `github.pull_request.opened`).
...
Other integrations with webhook-based triggers:

- Slack — app mentions and slash commands. See `integrations/slack.mdx`.
- Linear — assignment to Tembo and repository label application. See `integrations/linear.mdx`.
- GitLab — merge request and comment events. See `integrations/gitlab.mdx`.
- Bitbucket — pull request and comment events. See `integrations/bitbucket.mdx`.
- Jira — issue label application (e.g., “Tembo”). See `integrations/jira.mdx`.
- Sentry — error/issue events mapped to repositories. See `integrations/sentry.mdx`.

We’ll continue expanding these lists in the integration docs as new events are added. If you need a specific provider event wired to a workflow, contact hi@tembo.io.
