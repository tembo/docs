...
- Automation definition lives in your code repository under `/.tembo/workflows/*.json`.
- Tembo scans your repository and imports each automation into your organization.
- An automation can be started by a schedule (cron) or a trigger (an event), which enqueues a background job.
- The job records an Automation Run and converts your automation JSON into a Tembo “Automation Issue”, then Tembo executes it using the same engine that powers issue solving.

## Defining Automations in Your Repo

Add one or more JSON files in `/.tembo/workflows/`. Each file should contain at least a `name` and `jsonContent` (the editor JSON for your automation).
