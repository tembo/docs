...
Call [`POST /automation/{slug}/trigger`](/api/post-automation-slug-trigger) with any JSON payload.
...
You can trigger a task with the selected automation by using slash commands: `@tembo /<automation-slug>`.

```txt
@tembo /<automation-slug>
```
