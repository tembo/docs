...
Trigger any automation programmatically via the API. This lets you integrate Tembo into your own workflows, CI/CD pipelines, or any external system.

1. Set a **Key** on your automation (in the automation's properties panel)
2. Call the trigger endpoint with an arbitrary JSON payload:

```bash
curl -X POST https://api.tembo.io/public-api/automation/my-automation/trigger \
