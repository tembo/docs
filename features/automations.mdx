...
Use an `automationSlug` to trigger an automation on demand from external systems or chat commands.

1. Set a **Slug** on your automation (in the automation's properties panel).
2. Trigger the automation with that `automationSlug` from API calls or slash commands.
...
You can trigger an automation programmatically using its `automationSlug` via our SDK or an [API request](/api/trigger-automation).
