...
The Tembo self-hosted stack starts from a Tembo-provided NixOS AMI. The AMI is a base install image: it includes the operating system, SSH, and the installer service, then the installer downloads and applies the licensed Tembo runtime release.

After installation, services sit behind nginx on port 80:
...
Tembo distributes a pre-built NixOS AMI (Amazon Machine Image) to your AWS account. You launch an EC2 instance from that AMI, open the required ports, enter your license key in the installer, and configure a single JSON file. No OS setup or image building is required on your end.
...
<Note>
  Tembo's AWS base image uses legacy BIOS boot intentionally. This supports `c5.metal`, and virtualized Nitro instances such as `m8i` also boot correctly through SeaBIOS.
</Note>
...
| Custom TCP | TCP | 8080 | Your IP or load balancer | nginx health check |
| SSH | TCP | 22 | Your IP | SSH access |

Ports 3999 and 8888 are only needed during setup and manual updates. You can remove or restrict those rules after configuration is complete.
...
The installer stores its state in `/var/lib/flake-installer`. The release download step keeps the current extracted artifact under `downloads/current`, and the install step runs the release's NixOS build for the AWS self-hosted style.

The installer settings page is generated from the downloaded release. Current settings include hostname, optional VS Code Server, local Postgres, pgAdmin, Prometheus, Tembo Cron, Tailscale, config seeding, secret regeneration, worker count, and agent SSH key material for QEMU sandbox VMs.
