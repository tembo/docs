...
At a high level, self-hosted Tembo is a NixOS-based deployment with a base install image and an installer-driven runtime update flow.

If you want the simplest deployment model, Tembo **can run on a single VM**.

The base image contains the operating system, SSH access, the installer service, and cloud-specific boot configuration. It does not contain Tembo application code. During installation, the installer downloads the licensed Tembo release artifact, renders the release-provided settings schema, and runs the NixOS switch into the Tembo runtime configuration.

At a lower level, a standard runtime deployment runs these services:

- **Web app** for the user interface
- **API** for product workflows, auth, and orchestration
- **Admin UI** for operational inspection
- **Host manager and workers** for queue processing and sandbox lifecycle management
- **Sandbox runtime** for executing sessions in Docker or QEMU-backed isolated environments
- **Cron** for scheduled jobs (kicked off from tembo's `Agents` feature or via tool calls from coding agents)
- **Redis**
- **Postgres**
- **nginx** as the front door for the web app, API, admin UI, and installer
- **Prometheus** for local status and service alerts when monitoring is enabled
...
For releases and updates, Tembo can manage them for you, or your team can manually opt in to changes through the installer web UI that ships with the deployment by default. The installer supports both initial install and update workflows, stores runtime state under `/var/lib/flake-installer`, and writes Tembo runtime configuration to `/var/lib/tembo/config.json`.
...
- The base images ship without embedded Tembo application code or customer secrets
- SSH is key-based, with password authentication disabled

## Installer settings

The installer settings form is loaded from the downloaded Tembo release artifact, so the options can evolve with each release. Current self-hosted releases expose settings for:

- Hostname
- VS Code Server for remote config editing
- Local Postgres and pgAdmin
- Prometheus monitoring
- Tembo Cron
- Tailscale access
- Runtime config seeding and secret regeneration
- Worker count
- Agent SSH key material used by QEMU sandbox VMs
