...
| `applicationBaseUrl` | `ENVIRONMENT_URL` | `tembo-staging.com` | Environment URL |
...
| `betterAuth.enabledOauthProviders` | `BETTER_AUTH_ENABLED_OAUTH_PROVIDERS` | Comma-separated Better Auth OAuth providers to enable |
...
| Key | Env Var | Default | Description |
|-----|---------|---------|-------------|
| `oidc.privateKey` | `TEMBO_OIDC_PRIVATE_KEY` | — | RSA private key for signing OIDC tokens |
| `oidc.issuer` | `TEMBO_OIDC_ISSUER` | — | OIDC issuer URL, falling back to `api.base` when empty |
| `oidc.keyId` | `TEMBO_OIDC_KEY_ID` | `tembo-oidc-1` | OIDC signing key ID |
...
| Key | Env Var | Default | Description |
|-----|---------|---------|-------------|
| `sandbox.qemuPath` | `QEMU_PATH` | — | Optional path to `qemu-system-x86_64` |
| `sandbox.docker.image` | `SANDBOX_DOCKER_IMAGE` | `ghcr.io/tembo/monorepo/sandbox:latest` | Docker image |
...
| `sandbox.keepAlive.app` | `SANDBOX_KEEP_ALIVE_APP_MINUTES` | `30` | Minutes to keep an app sandbox alive after completion |
| `sandbox.keepAlive.slack` | `SANDBOX_KEEP_ALIVE_SLACK_MINUTES` | `5` | Minutes to keep a Slack-origin sandbox alive after completion |
| `sandbox.computerPreview.maxAttempts` | `SANDBOX_COMPUTER_PREVIEW_MAX_ATTEMPTS` | `30` | Max attempts while polling for computer preview |
| `sandbox.computerPreview.retryDelayMs` | `SANDBOX_COMPUTER_PREVIEW_RETRY_DELAY_MS` | `2000` | Delay between computer preview polling attempts |
...
| `sandbox.sizes.large.memoryMb` | `SANDBOX_LARGE_MEMORY_MB` | `16384` | Memory (MB) for large |
...
| `sandbox.sizes.xxl.cpu` | `SANDBOX_XXL_CPU` | `16` | vCPUs for xxl sandboxes |
| `sandbox.sizes.xxl.diskSizeGb` | `SANDBOX_XXL_DISK_SIZE_GB` | `200` | Disk (GB) for xxl |
| `sandbox.sizes.xxl.memoryMb` | `SANDBOX_XXL_MEMORY_MB` | `65536` | Memory (MB) for xxl |
...
| `sandbox.sizes.xl.memoryMb` | `SANDBOX_XL_MEMORY_MB` | `32768` | Memory (MB) for xl |
...
| `queue.jobExecutionTimeout` | `QUEUE_JOB_TIMEOUT` | `86400000` (24h) | Max runtime for jobs (ms) |
...
| `agent.sshPublicKey` | `AGENT_SSH_PUBLIC_KEY` | — | SSH public key used by agents to authenticate into QEMU sandbox VMs |
...
| Key | Env Var | Default | Description |
|-----|---------|---------|-------------|
| `manager.apiUrl` | `MANAGER_API_URL` | `http://127.0.0.1:8743/` | Local host manager API URL used by workers on the same host |
...
| `mcp.temboMcpExecutable` | `TEMBO_MCP_EXECUTABLE` | `/run/current-system/sw/bin/tembo-mcp` | Path to the tembo MCP executable |
...
| `aws.oidcTemplateUrl` | `AWS_OIDC_CROSS_ACCOUNT_TEMPLATE_URL` | `https://tembo-use1-dev-cfn-templates.s3.us-east-1.amazonaws.com/tembo-identity.yaml` | CloudFormation template URL for AWS OIDC integration |
| `aws.selfHostedReleases.bucketName` | `SELF_HOSTED_RELEASES_BUCKET` | — | S3 bucket for self-hosted releases (`AWS_SELF_HOSTED_RELEASES_BUCKET_NAME` is deprecated) |
| `aws.selfHostedReleases.rcBucketName` | `SELF_HOSTED_RELEASES_RC_BUCKET` | — | S3 bucket for release candidate self-hosted releases |
...
| Key | Env Var | Default | Description |
|-----|---------|---------|-------------|
| `selfHosted.configPath` | `TEMBO_CONFIG_PATH` | `/var/lib/tembo/config.json` | Path to the self-hosted JSON config file |
| `selfHosted.checkUpdateEndpoint` | `FLAKE_INSTALLER_CHECK_UPDATE_ENDPOINT` | `https://internal.tembo.io/self-hosted/check-update` | Installer update check endpoint |
| `selfHosted.licenseKeyFile` | `TEMBO_LICENSE_KEY_FILE` | `/var/lib/flake-installer/.installer/license-key.txt` | Path to the license key file on self-hosted installs |

## Assets

| Key | Env Var | Default | Description |
|-----|---------|---------|-------------|
| `assets.storage` | `ASSETS_STORAGE` | — | Bucket or container for agent-produced assets |
| `assets.cdn.domain` | `ASSET_CDN_DOMAIN` | — | CDN domain for agent assets |
| `assets.cdn.keyId` | `CLOUDFRONT_ASSETS_KEY_ID` | — | CDN key pair ID for signing asset URLs |
| `assets.cdn.privateKey` | `CLOUDFRONT_ASSETS_PRIVATE_KEY` | — | CDN private key PEM for signing asset URLs |
...
```json
{
  ...
  "sandbox": {
    "docker": {
      "image": "ghcr.io/tembo/monorepo/sandbox:latest"
    }
  }
}
```
