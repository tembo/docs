...
Tembo Hooks allow you to customize and extend Tembo's behavior by running custom commands at specific points during the issue resolution workflow. This powerful feature enables you to integrate your existing development practices, CI/CD processes, and quality checks directly into Tembo's automated workflow.
...
- Run additional CI/CD processes
...
```json
{
  "hooks": {
    "onFinish": [
      "slack-notify 'Tembo created PR: {{PR_URL}}'",
      "update-jira-ticket {{ISSUE_ID}}",
      "npm run cleanup"
    ]
  }
}
...
- For `onFinish` hooks, failures are logged but don't affect the overall success of the solution
...
```json
{
  "hooks": {
    ...
    "onFinish": [
      "docker-compose down",
      "npm run cleanup",
      "echo 'Tembo solution completed for {{ISSUE_TITLE}}'"
    ]
  }
}
```
...
```json
{
  "hooks": {
    ...
    "onFinish": [
      "make clean",
      "echo 'Go application solution completed'"
    ]
  }
}
```
...
Tembo Hooks are designed to complement your existing development practices:

- **CI/CD Integration**: Use hooks to trigger existing pipeline steps
- **Code Quality**: Integrate with tools like SonarQube, CodeClimate, or custom analyzers
- **Team Notifications**: Send updates to Slack, Discord, or email systems
- **Project Management**: Update tickets in Jira, Linear, or other PM tools

By leveraging Tembo Hooks, you can ensure that Tembo's automated solutions follow your team's established practices and quality standards while maintaining the speed and efficiency of AI-driven development.