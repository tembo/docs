...
Tembo prepares an environment for each selected session size. Each project build captures both disk and memory, including running processes. When you start a session from that project, Tembo clones this prepared environment instead of cloning repositories and installing dependencies from scratch. This is especially useful for large repositories, monorepos, and projects with slow dependency installs.
...
Processes that your setup script starts and leaves running are also preserved in every session that boots from that project build. For example, you can start your whole app during setup—its frontend development server, backend API, and background workers—so each session starts with the app already running instead of having to launch it again. Start long-running processes in the background so the setup script can finish and Tembo can capture the environment.
