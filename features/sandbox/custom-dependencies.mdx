...
Because the toolchain is captured at build time, edits to `tembo.nix` do not reach project sessions until the environment is rebuilt. Rebuild the project, or—if **Refresh every day** is enabled—wait for its next scheduled refresh.
