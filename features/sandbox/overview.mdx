...
Tembo offers six VM sandbox sizes. Each session runs in a dedicated Linux VM, and no two sessions share the same VM.
...
Nano, Micro, and Medium are best for routine code analysis, fixes, features, and reviews. Large, XL, XXL, and Ultra are best for heavier builds, integration tests, Docker workloads, large repositories, or multi-container setups.

<Note>
  Large, XL, XXL, and Ultra sandboxes require large VM access. If these sizes are not enabled for your workspace, they appear locked in size selectors. Request access from workspace settings.
</Note>
