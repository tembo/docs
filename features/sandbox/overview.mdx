...
Tembo offers seven VM sandbox sizes. Each session runs in a dedicated Linux VM, and no two sessions share the same VM.
...
Nano, Micro, Medium, and XL are best for routine code analysis, fixes, features, and reviews. Large, XXL, and Ultra are best for heavier builds, integration tests, Docker workloads, large repositories, or multi-container setups.

<Note>
  Large, XXL, and Ultra sandboxes may require access before they are available. If these sizes are locked in size selectors, request access from **Settings > Larger VMs**.
</Note>
