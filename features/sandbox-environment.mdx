...
| **yarn** | 1.22.x | Popular JavaScript package manager |
...
Tembo's sandbox environment can also run your task inside a **Dev Container** (the same `.devcontainer` format used by VS Code Dev Containers and GitHub Codespaces). This is useful when your project needs OS packages, language runtimes, or tooling that isn't already available in the base sandbox.

**Requirements:**
- Use the **Large Full VM** sandbox type (Dev Containers require Docker)
- Dev Containers are not supported in the **Small** sandbox type
- Commit a Dev Container configuration at `.devcontainer/devcontainer.json` in your repository root

**How it works:**
- Tembo detects `.devcontainer/devcontainer.json`
- Tembo starts the container for your repo
- All subsequent commands run *inside* that container environment

**Minimal example: `.devcontainer/devcontainer.json`**
```json
{
  "name": "tembo-sandbox",
...
**Tips:**
- Keep `postCreateCommand` **idempotent** and **fast** (it runs during container setup).
- If your Dev Container build is slow or flaky, the task will be slow or fail early—test it locally with VS Code's "Dev Containers: Reopen in Container" before relying on it in Tembo.
- For multi-repo tasks, only one Dev Container can be used per run; Tembo uses the first repo that has a `.devcontainer/devcontainer.json`.
...
| Repository uses Dev Containers | Large Full VM |
