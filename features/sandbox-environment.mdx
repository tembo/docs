...
- **Nested virtualization**: Run Docker containers inside the sandbox (Docker-in-Docker)
...
**Note**: To run Docker containers inside the sandbox (e.g., for testing with Postgres, Redis, or other services), you must use the **Large VM** sandbox type.
...
The Large VM sandbox supports running Docker containers inside the environment, enabling you to test and build containerized applications. This is useful for:

- Running integration tests with databases (Postgres, MySQL, Redis, etc.)
- Building Docker images as part of your CI/CD workflow
- Testing multi-container applications with Docker Compose

### Example: Running Postgres in Docker

To run a Postgres database inside the sandbox for testing:
...
| Running tests with Docker databases | Large VM |
| Building Docker images | Large VM |
...
- **Use Large VM for Docker workloads**—Standard sandbox cannot run nested containers
