...
| Feature | Standard (Default) | Large VM |
|---------|-------------------|----------|
| **Startup** | Fast, lightweight container | Full VM with nested virtualization |
| **Docker Support** | No | Yes (run containers inside sandbox) |
| **Isolation** | Container-level | Full VM-level |
| **Best For** | General development, web apps | Docker-based testing, multi-container setups |

**Note**: Use **Large VM** when agents need to run Docker containers (e.g., for testing with Postgres, Redis, or other services).
