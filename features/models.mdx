...
### Connect a workspace ChatGPT subscription

Org admins can connect a single ChatGPT Codex subscription for the whole workspace, instead of (or in addition to) each member connecting their own.

1. Open **Settings > Models**.
2. Under **Subscriptions**, click **Connect Codex**.
3. Copy the authorization code, then click **Open authorization page**.
4. Enter the code in ChatGPT and approve the connection. Keep the Tembo dialog open while authorization completes.

A workspace subscription is the default for supported OpenAI models for all workspace users. If a member has their own personal Codex subscription connected, Tembo prefers that member's personal subscription over the workspace one.

## Reset Codex usage

If your connected ChatGPT subscription has an available usage reset, open the **Connected** dropdown next to ChatGPT Codex in **Settings > Connected accounts** (or **Settings > Models** for a workspace subscription) and select **Use reset** to reset eligible Codex usage limits immediately. This uses one of the subscription's available resets and cannot be undone. For a workspace subscription, only org admins can redeem a reset.
...
| Fable 5.1 | `claude-fable-5-1` | Anthropic | No |
...
| GPT-6 Astra | `gpt-6-astra` | OpenAI | Yes |
...
| DeepSeek V4.1 Flash | `deepseek-v4p1-flash` | DeepSeek | No |
