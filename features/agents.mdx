...
If you need to transform the payload or fetch extra data before the agent starts, use an [agent post-trigger hook](/beta/agent-pre-run-script). The hook can read `TRIGGER_PAYLOAD` and print Markdown, text, or JSON to standard output; Tembo adds that output to the agent's starting context.
