...
Tembo is agent and model agnostic. For each agent you configure, you pick the coding agent (and model) that runs in the session it spawns, for example Claude Code with Sonnet 4.6, Codex with GPT-5.4, or Cursor. Set a default per agent, or override per task.
...
4. Select [coding agent](#coding-agent)
