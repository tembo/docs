...
async function createTask(taskData: CreateTaskRequest) {
  const response = await fetch('https://api.tembo.io/public-api/task/create', {
    ...
    body: JSON.stringify({
      ...
      json: taskData.json,
      queueRightAway: taskData.queueRightAway || false,
      codeRepoIds: taskData.codeRepoIds || [],
      branch: taskData.branch || null,
      agent: taskData.agent || 'claudeCode:claude-4-sonnet',
    }),
  });
...
const taskData: CreateTaskRequest = {
  ...
  json: JSON.stringify({ priority: 'high', type: 'bug' }),
  queueRightAway: true,
