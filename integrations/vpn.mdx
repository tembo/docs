...
- Supports Fortinet SSL VPN, Cisco AnyConnect, Palo Alto GlobalProtect, and Juniper Network Connect
...
<Steps>
  ...
  <Step title="Enter your VPN details">
    ...
    - **Protocol** — select your VPN type (Fortinet, Cisco AnyConnect, Palo Alto GlobalProtect, or Juniper)
...
1. Configure the VPN integration with your Fortinet gateway credentials
...
<AccordionGroup>
  <Accordion title="Supported protocols">
    The VPN integration uses [OpenConnect](https://www.infradead.org/openconnect/), which supports:
    - **Fortinet SSL VPN** (`fortinet`) — compatible with FortiClient
    - **Cisco AnyConnect** (`anyconnect`) — compatible with Cisco Secure Client
    - **Palo Alto GlobalProtect** (`gp`) — compatible with GlobalProtect client
    - **Juniper Network Connect** (`nc`) — compatible with Juniper/Pulse Secure
  </Accordion>
