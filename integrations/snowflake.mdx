...
FROM SPECIFICATION $$
instructions:
  response: "When the user asks about Tembo sessions, repos, or automations, use the Tembo MCP tools."
mcp_servers:
  - server_spec:
...
Grant the role(s) that will use the agent access to the MCP server, the UDF, the external access integration, and the agent itself:
