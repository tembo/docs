...
- The Tembo REST API base URL: `https://api.tembo.io/v1`
...
<AccordionGroup>
  <Accordion title="Path A: External MCP Connector (OAuth2) — not yet supported">
    ...
    CREATE OR REPLACE API INTEGRATION tembo_mcp_api_integration
      ...
      API_ALLOWED_PREFIXES = ('https://api.tembo.io/v1/mcp')
  ...
  <Accordion title="Self-hosted Tembo">
    For self-hosted Tembo, replace `api.tembo.io:443` in the network rule with your deployment's host and set the Python UDF's `BASE` to `https://your-host.example.com/api/public-api/v1`. The rest of the flow is identical.
  </Accordion>
