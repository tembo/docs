...
Tembo acts as an OIDC identity provider. Agents request short-lived STS credentials on demand from any session or automation in your workspace — no long-lived AWS access keys are stored in Tembo.
...
Once connected, agents can run AWS CLI commands and programs that use an AWS SDK. Each enabled integration can be configured as a standard AWS profile inside the agent sandbox.

Agents can list the accounts available to your workspace:
...
```bash
tembo aws exec staging -- aws sts get-caller-identity
tembo aws exec production -- aws s3 ls
```

The selector can be the integration's environment label, generated profile name, AWS account ID, or integration ID. If two integrations use the same label or AWS account ID, use the generated profile name shown by `tembo aws profiles`.
...
6. The credential helper caches those temporary credentials in the sandbox and refreshes them when needed while the agent run remains active and the integration remains enabled.
...
<AccordionGroup>
  ...
  <Accordion title="No AWS integrations are available">
  ...
  <Accordion title="An AWS profile selector is ambiguous or not found">
  ...
  <Accordion title="Credentials stop refreshing during a long-running command">
    Credentials refresh automatically while the agent run remains active. Refresh fails closed if the run ends or the AWS integration is disabled, removed, or no longer belongs to the workspace. Start a new active session or re-enable the integration, then retry the command.
  </Accordion>

  <Accordion title="Agent can't reach AWS APIs">
    The agent sandbox requires outbound HTTPS access to AWS STS and the AWS service endpoints used by the command. On self-hosted deployments in a private network, confirm that this egress is allowed.
  </Accordion>
