...
- **`agent=`** - Specify which coding agent and model to use (format: `agentType:model`)
  - Example: `agent=claudeCode:claude-opus-4-5`, `agent=codex:gpt-5.4`
...
See [Coding Agents](/features/coding-agents) for available models. Quick examples: `agent=codex:gpt-5.4` for OpenAI-powered work, `agent=claudeCode:claude-4-5-haiku` for fast fixes, `agent=cursor:grok` for complex refactoring.
