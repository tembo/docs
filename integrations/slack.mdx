...
<Steps>
  ...
  <Step title="Authorize Slack Permissions">
    Authorize Tembo to access your Slack workspace. The bot requests permissions to:

    - **Read messages** in channels, DMs, and threads (for conversation context)
    - **Send messages** and status updates (including DMs and reactions)
    - **Share files** and code snippets
...
Without an `agent=` option, Tembo uses your organization's default (configurable in [Settings](https://app.tembo.io/settings)). Individual repositories can override this default.

See [Coding Agents](/features/coding-agents) for available models. Quick examples: `agent=claudeCode:claude-4-5-haiku` for fast fixes, `agent=cursor:grok` for complex refactoring.
...
Tasks created from Slack integrate with the [Feedback Loop](/features/feedback-loop). When you review a Tembo PR on GitHub or GitLab and mention `@tembo` in your comments, Tembo processes your feedback, updates the PR, and **posts a notification back to the original Slack thread** with a summary of changes and links to view the updated PR. Status reactions (⌛️ processing, ✅ complete) keep you informed throughout.

## Best Practices

- **Write clear, specific prompts** with file paths and context (e.g., "Add error handling to the user registration endpoint" not "Fix the bug"). See the [Prompting Guide](/prompting-guide) for detailed tips.
- **Use threads for collaboration** — discuss the problem in a thread, then mention `@tembo` with the final task description. Tembo reads the full thread context.
...
The Tembo Slack integration collects and processes data in accordance with our [Privacy Policy](https://tembo.io/privacy). This includes:

- Message content when you mention `@tembo` (used to process your task requests)
- Thread context for conversation history (to provide relevant responses)
- User information for personalization and attribution

For questions about how your data is handled, please review our full [Privacy Policy](https://tembo.io/privacy).

## Support

Need help with the Slack integration? Contact us at [support@tembo.io](mailto:support@tembo.io).
