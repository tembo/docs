...
Once connected, Tembo continuously monitors your PostgreSQL database for:

### Performance Issues

- **Slow Queries** - Analyzes `pg_stat_statements` for queries with >100ms execution time and >5 calls
- **Missing Indexes** - Identifies tables that would benefit from indexing based on query patterns
- **Unused Indexes** - Detects indexes consuming space without being utilized

### Database Health

- Connection counts and session activity
...
- `pg_stat_statements` extension is enabled for query monitoring
