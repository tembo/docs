...
- **Use read-only access** — Tembo only needs read access and never modifies your database directly
- **Connect production databases** — Get the most accurate performance insights from real traffic
- **Review migration PRs** — Always review suggested migrations before merging, especially for large tables
- **Enable pg_stat_statements** — Required for query analysis. Run `CREATE EXTENSION IF NOT EXISTS pg_stat_statements;`
