...
## Writing Effective Jira Tickets

When creating issues for Tembo to work on, provide clear and actionable descriptions:

**Good Examples:**
- "Fix null pointer exception in UserService.getProfile() when user has no avatar set"
- "Add rate limiting to /api/users endpoint - max 100 requests per minute per API key"
- "Refactor PaymentProcessor to use strategy pattern for different payment providers"
...
1. **Summary** - Clear, specific one-line description
2. **Description** - Detailed context about the problem or feature
3. **Acceptance Criteria** - Specific conditions that define "done"
4. **Technical Notes** - Relevant file paths, error messages, or implementation hints
