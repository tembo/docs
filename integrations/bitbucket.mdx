...
Tembo listens to common Bitbucket events and maps them to workflow triggers (reference them as `bitbucket.<category>` in your `triggerName`, e.g., `bitbucket.pull_request.opened`). The Integrations page in the app shows the live set enabled for your org. Supported events:
