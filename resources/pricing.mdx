...
[Projects](/features/projects) are available on every plan. Free includes one active project per workspace; paid plans include unlimited projects. Upgrade in **Settings** > **Billing** to create more projects.
...
| Tembo | `deepseek-v4p1-flash` | $0.253 | $0.759 | n/a | $0.00805 |
...
- **Free**: new sessions are blocked until you upgrade or add prepaid balance. After each model call, Tembo checks the allowance and stops the running session if it has been exceeded, with a message in the chat explaining why.
...
When work is blocked, the request is rejected immediately rather than queued. On Pro and Max, work that has already started can continue its current session so its compute and inference are accounted for correctly. On Free, Tembo checks after each model call and stops a running session if the allowance has been exceeded, rather than allowing it to continue to completion.
